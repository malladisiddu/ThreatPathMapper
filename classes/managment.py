...
class Manager():
    ...
    def analyze_big_campaign(self,dir):
        ...
        supported_extensions = ['.txt', '.json', '.html', '.htm', '.pdf']
        ...
        for index in range(0, len(files)):
            ...
            if file_ext:
                # Extract campaign_id (remove extension)
                campaign_id = file[:file.rfind('.')]
                print(f"start analyzing report: {campaign_id} (type: {file_ext})")
                
                try:
                    big_campaign = BigCampaign(file_path, campaign_id)
                    ...
                    print(f"Successfully processed {file} -> {len(big_campaign.data)} campaign chunks")
    ...
    def analyze_url_campaign(self, url: str, campaign_id: str = None):
        """Analyze CTI report from URL"""
        if not campaign_id:
            # Generate campaign_id from URL
            from urllib.parse import urlparse
            parsed = urlparse(url)
            campaign_id = f"url_{parsed.netloc}_{parsed.path.replace('/', '_')}"[:50]
        
        print(f"start analyzing report from URL: {campaign_id}")
        
        try:
            big_campaign = BigCampaign(url, campaign_id)
            if len(big_campaign.data) == 0:
                print(f"No content extracted from URL: {url}")
                return None
            big_campaign.to_jsonl(os.path.join(campaigns_output_dir, campaign_id + ".jsonl"))
            self.big_campaigns.append(big_campaign)
            print(f"Successfully processed URL -> {len(big_campaign.data)} campaign chunks")
            return big_campaign
        except Exception as e:
